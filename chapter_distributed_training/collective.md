...
#### AllGather
相比起Gather，AllGather 算子会把聚合的结果存到所有的处理单元上。在这里，一个简单的做法是使用Gather和Broadcast把聚合结果先存到编号为1的处理单元中，再将其广播到剩余的处理单元上。这会产生一个a log p + (p-1) bl + (a+plb) log p的时间复杂度，因为在Broadcast时如果忽略链表/数组实现所带来的额外空间开销，每次通信的长度为pl而不是l。简化后，我们得到了一个a log p + p log p lb 的时间复杂度。在一个更复杂的算法下，我们可以将其进一步优化到和Gather一样的a log p + (p-1) bl （:cite:`Sanders2019-cq`），然而由于篇幅问题便不再赘述。
